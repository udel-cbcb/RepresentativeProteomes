...
foreach(@loop) {
	$Xin = $_;
	$rpgFile = "../results_corr_consist/$Xin/rpg"."-".$Xin.".txt";
	$ppFile = "../results_corr_consist/$Xin/pp"."-".$Xin.".txt";
	$ppFileOrig = "../results_corr_consist/$Xin/pp"."-".$Xin.".txt.orig";
	#`cp $ppFile $ppFileOrig`;
	...
	while($line=<RPG>) {
		if($line =~ /^>/) {
			...
			$key1 = $upId."-".$taxId;
			$ppRPGs{$key1} = 1;
			$proteome1 = $proteomesHash{$key1};
			my $taxonomyId = $proteome1->getTaxId();
			$uniRef50sRef1 = $proteome1->getUniRef50s();
                	%uniRef50s1 = %$uniRef50sRef1;
                	my $membersRef1 = $proteome1->getMembers();
                	my %members1 = %$membersRef1;
                	my $upIdAndTaxId1 = $proteome1->getUPIdAndTaxId();
			#print "RP ".$key1."\t".$upIdAndTaxId1."\n";
                	for my $m1 (keys %members1) {
                        	$upIdAndTaxEntryHash{$upIdAndTaxId1}{$upIdAndTaxId1}{$m1} = 1;
                        	$panMembers{$m1} = $upId."\t".$taxId."\t".$entryProteomeHash{$m1}{$upIdAndTaxId1};
                        	#print ">$m1= ".$upId."\t".$taxId."\t".$entryProteomeHash{$m1}{$upIdAndTaxId1}."\n";
                	}
                        if($Xin == 95) {
			...
			#print $key1."\t".keys(%panMembers)."\n";
			#$proteomesHash{$key1} = $proteome1;
			#print $key1."\t".$proteome1->getUPIdAndTaxId()."\n";
                        my $upIdAndTaxId1 = $proteome1->getUPIdAndTaxId();
                        ...
                        for $ac (sort keys %panMembers) {
                                print PP " ".$ac."\t".$panMembers{$ac}."\n";
                        }
			print PP "\n";
		}
		else {	
			...
			my $key2 = $upId."-".$taxId;
 			...
 			my $uniRef50sRef2 = $proteome2->getUniRef50s();
                        ...
                        my $upIdAndTaxId2 = $proteome2->getUPIdAndTaxId();
                        for my $uniRefAc (keys %uniRef50s2) {
                        	if($uniRef50s1{$uniRefAc} eq "") {
                                	my $proteinsRef = $proteomeUniRefEntryHash{$upIdAndTaxId2}{$uniRefAc};
                                        my %proteins = %$proteinsRef;
                                        ...
                                        if($protein) {
                                        	my $ac = $protein->getAC();
                                                if($panMembers{$ac} eq "") {
                                                	...
                                                	#print " $ac = $upId\t$taxId\t$uniRefAc\n";
                                                        $upIdAndTaxEntryHash{$proteome1->getUPIdAndTaxId()}{$upIdAndTaxId2}{$ac} = 1;
...
sub computeRPGAndPanProteome {
	...
	foreach my $k1 (sort keys (%rpgNameHash)) {
		...
		if($panMemberSize > 0) {
			...
			for my $ac (sort keys %panMembers) {
				print PP " ".$ac."\t".$panMembers{$ac}."\n";
			}
			print PP "\n";
		}		
