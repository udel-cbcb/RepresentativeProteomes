...
if(@ARGV != 1) {
        print "perl getScoresInc.pl prevRP75.txt\n";
        exit 1;
}
			...
			if($lineage !~ /^Viruses\;/) {
				...
				foreach(@upIds) {	
					...
					if($upCount{$upId."\t".$taxId} > 100) {
						...
						print "Less than or equal to 100 CP: $upId\t$taxId\n";
					}
				}
			}
			else {
				print "Virues CP: $upId\t$taxId\n";
			}
		}
                $firstAC="";
		$pmidScore = 0;
...
open(PSCORE, ">", "../data/score_inc/proteomeScores.txt");
