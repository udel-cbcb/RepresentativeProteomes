...
sub computeProteomesCorMatrix {
	...
	for(my $i = 0; $i < @taxIds; $i++) {
		if(($i % $numThreads)+1 == $num) {	
			...
			for(my $j = 0; $j < @taxIds; $j++) {
				$taxId2 = $taxIds[$j];
				if(!($sameProteomes{$taxId1} && $sameProteomes{$taxId2})) {
					...
					if($taxId1 eq $taxId2) {
						...
						if($ASum < $BSum) {
							...
							$log .=  "min $num 100*$ABSum/$BSum = ".$miny."\n";
                                                	...
                                                	$log .=  "min ".$num." ".$taxId2. " vs ".$taxId1." : miny = ".$miny."\t$ABSum(AB) $BSum(B) + $ASum(A)\n";
                                        		$threadProteomesCorMatrixHashMin{$taxId1}{$taxId2} = $minx;
                                                	$log .= "min ".$num." A < B ".$taxId1. " vs ".$taxId2." : ".$minx."\n";
                                        		$threadProteomesCorMatrixHashMin{$taxId2}{$taxId1} = $miny;
                                                	$log .= "min ".$num." A < B ".$taxId2. " vs ".$taxId1." : ".$miny."\n";
						}
						else {
							$minx = 100*$ABSum /$BSum;
							$miny = 100*$ABSum /$ASum;
							...
							$log .= "min $num 100*$ABSum/$ASum = ".$miny."\n";
                                                	...
                                                	$log .= "min ".$num." ".$taxId2. " vs ".$taxId1." : minx = ".$minx."\t$ABSum(AB) $BSum(B) + $ASum(A)\n";
                                        		$threadProteomesCorMatrixHashMin{$taxId1}{$taxId2} = $miny;
                                                	$log .= "min ".$num." A >= B ".$taxId1. " vs ".$taxId2." : ".$miny."\n";
                                        		$threadProteomesCorMatrixHashMin{$taxId2}{$taxId1} = $minx;
                                                	$log .= "min ".$num." A >= B ".$taxId2. " vs ".$taxId1." : ".$minx."\n";
						}
						my $x = (200*$ABSum)/($ASum+$BSum);
                                                $log .= $num." ".$taxId1. " vs ".$taxId2." : x = ".$x."\t$ABSum(AB) $ASum(A) + $BSum(B)\n";
                                                $log .= $num." ".$taxId2. " vs ".$taxId1." : x = ".$x."\t$ABSum(AB) $BSum(B) + $ASum(A)\n";
	...
	open(TCORRMIN, ">>", "../data/min_thread_".$num."_corr.txt");
