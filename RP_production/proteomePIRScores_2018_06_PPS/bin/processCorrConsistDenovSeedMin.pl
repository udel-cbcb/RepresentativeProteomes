...
#axon   #PMID   #PDB    #SwissProt      ScoreSum        TotalEntries    ScoreSum/TotalEntries
#100379  5       0       137     1111.17642732323        730     1.52215948948388
...
while($line = <PSCORE>) {
	...
	if($line !~ /^UPID/) {
		...
		if($rec[8] eq "RefP") {
                        $pps{$upIdAndTaxId} ="PPS:1,".$prevRP.",".$rec[2].",".$rec[3].",".$rec[4].",".$rec[6];
                        $pps95{$upIdAndTaxId} ="PPS:1,0,".$rec[2].",".$rec[3].",".$rec[4].",".$rec[6];
                }
                else {
                        $pps{$upIdAndTaxId} ="PPS:0,".$prevRP.",".$rec[2].",".$rec[3].",".$rec[4].",".$rec[6];
                        $pps95{$upIdAndTaxId} ="PPS:0,0,".$rec[2].",".$rec[3].",".$rec[4].",".$rec[6];
                }
	}
}
close(PSCORE);
...
foreach my $upIdAndTaxId (sort keys %upIdAndTaxIdScoresHash)  {
	open(SCORE, "<", "../data/score/$upIdAndTaxId"."_score.txt") or die "Can't open ../data/score/$upIdAndTaxId"."_score.txt\n";
	while($line = <SCORE>) {
       		chomp($line);
		if($line !~ /^Accession/) {
        		my @rec = split(/\t/, $line);
...
foreach(@loop) {
	if($_ == 95) {
		...
		computeRPGAndPanProteome($_, %inputProteomesScoreHash);
	}
	else {
