...
while($line=<RP>) { 
	if($line =~ /^>/) {
		#print $line;
		chomp($line);
		my ($uniprot_taxon_id, $uniprot_os_code, $uniprot_name, $taxon_group, $score, $cutoff, $refp) = (split(/\t/, $line))[0, 1, 2, 3, 4, 5, 6];
		$uniprot_taxon_id =~ s/^>//;
		$gp_id = $gp2rp{$uniprot_taxon_id};
		$refSeq = $refSeq2rp{$uniprot_taxon_id};
		print ">".$gp_id."\t".$refSeq."\t".$gpInfo{$gp_id}."\t".$uniprot_taxon_id."\t".$uniprot_os_code."\t".$uniprot_name."\t".$taxon_group."\t".$score."\t".$cutoff."\t".$refp."\n";	
	}
	elsif($line =~ /^ /) {
		#print $line;
		chomp($line);
		my ($uniprot_taxon_id, $uniprot_os_code, $uniprot_name, $taxon_group, $score, $cutoff, $refp) = (split(/\t/, $line))[0, 1, 2, 3, 4, 5, 6];
		$uniprot_taxon_id =~ s/^\s+//;
		$gp_id = $gp2rp{$uniprot_taxon_id};
		$refSeq = $refSeq2rp{$uniprot_taxon_id};
		print " ".$gp_id."\t".$refSeq."\t".$gpInfo{$gp_id}."\t".$uniprot_taxon_id."\t".$uniprot_os_code."\t".$uniprot_name."\t".$taxon_group."\t".$score."\t".$cutoff."\t".$refp."\n";	
