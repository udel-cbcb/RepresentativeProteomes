...
%upIdAndTaxIdScoresHash_ALL;
%upIdAndTaxIdScoreStrHash_ALL;
%upIdAndTaxIdEntryTotalHash_ALL;
%upIdAndTaxIdScore95Hash_ALL;
print "Getting proteome score all ...\n";
open(PSCORE, "<", "../data/score_inc_all/proteomeASScores.txt") or die "Can't open ../data/score_inc_all/proteomeASScores.txt\n";
while($line = <PSCORE>) {
        chomp($line);
        if($line !~ /^UPID/) {
                @rec = split(/\t/, $line);
                ...
                $entryTotal = $rec[6];
                $pScore = $rec[10];
                $upIdAndTaxIdEntryTotalHash_ALL{$upIdAndTaxId} = $entryTotal;
                $upIdAndTaxIdScoresHash_ALL{$upIdAndTaxId} = $pScore;
                $refp_ALL{$upIdAndTaxId} = $rec[2];
                if($rec[3] eq "PrevRP") {
                        $prevRP = 1;
                        $upIdAndTaxIdScoresHash95_ALL{$upIdAndTaxId} = $pScore - 8000;
                }
                else {
                        $prevRP = 0;
                        $upIdAndTaxIdScoresHash95_ALL{$upIdAndTaxId} = $pScore;
                }
                if($rec[2] eq "RefP") {
                        $pps_ALL{$upIdAndTaxId} ="PPS:1,".$prevRP.",".$rec[4].",".sprintf("%.2f", $rec[5]).",".$rec[6];
                        $pps95_ALL{$upIdAndTaxId} ="PPS:1,0,".$rec[4].",".sprintf("%.2f", $rec[5]).",".$rec[6];
                }
                else {
                        $pps_ALL{$upIdAndTaxId} ="PPS:0,".$prevRP.",".$rec[4].",".sprintf("%.2f", $rec[5]).",".$rec[6];
                        $pps95_ALL{$upIdAndTaxId} ="PPS:0,0,".$rec[4].",".sprintf("%.2f", $rec[5]).",".$rec[6];
                }
        }
}
close(PSCORE);
print "Total upIdAndTaxIdScoresHash_ALL: ".keys(%upIdAndTaxIdScoresHash_ALL)."\n";
print "Getting proteome score ALL ...done\n";
...
my %proteomesHash_ALL = ();
print "Getting proteome info ALL ...\n";
open(TAX, "<". "../data/up-taxonomy-complete_yes.tab") or die "Can't open ../data/up-taxonomy-complete_yes.tab\n";
while($line=<TAX>) {
        chomp($line);
        if($line !~ /^UPID/) {
                ...
                my $proteome;
                if($lineage =~/^Viruses/) {
                        if($upIdAndTaxIdScoresHash_ALL{$upIdAndTaxId} ne "") {
                                $proteome  = Proteome->new($rec[0]."-".$rec[1], $rec[0], $rec[1], $mnemonic, $scientificName, $lineage, \%uniRef50s);
                                $proteomesHash_ALL{$upIdAndTaxId} = $proteome;
                        }
                }
        }
}
close(TAX);
print "Total ProteomesHash: ".keys(%proteomesHash_ALL)."\n";
print "Getting proteome info ...done\n";
...
my %proteinsHash_ALL = ();
print "Getting entry score ALL ...\n";
foreach my $upIdAndTaxId (sort keys %upIdAndTaxIdScoresHash_ALL)  {
        open(SCORE, "<", "../data/score_inc_all/$upIdAndTaxId"."_AS.txt") or die "Can't open ../data/score_inc_all/$upIdAndTaxId"."_AS.txt\n";
        while($line = <SCORE>) {
                chomp($line);
                if($line !~ /^Accession/) {
                        my @rec = split(/\t/, $line);
                        ...
                        $protein = Protein->new($ac);
                        $protein->setScore($score);
                        $proteinsHash_ALL{$upIdAndTaxId}{$ac} = $protein;
                }
        }
        close(SCORE);
}
...
my %proteomeUniRefEntryHash_ALL = ();
open(UNIREF, "<", "../data/uniref50_ALL.dat") or die "Can't open ../data/uniref50_ALL.dat\n";
print "Reading uniRef50 ALL ...\n";
while($line =<UNIREF>) {
        chomp($line);
        @rec = split(/\t/, $line);
        $ac = $rec[0];
        $upIdAndTaxId = $rec[1]."-".$rec[2];
        $uniRefAc = $rec[3];
        if($proteinsHash_ALL{$upIdAndTaxId}{$ac}) {
                $proteomeUniRefEntryHash_ALL{$upIdAndTaxId}{$uniRefAc}{$ac} = $proteinsHash_ALL{$upIdAndTaxId}{$ac};
                $entryProteomeHash_ALL{$ac}{$upIdAndTaxId} = $uniRefAc;
                $count++;
                if($count % 1000000 eq 0) {
                        $date = `date`;
                        print $date;
                        print "UniRef50 ALL read ".$count." .. done\n";
                }
        }
}
close(UNIREF);
print "Reading uniRef50 ALL ... done\n";

my %proteomesScoreHash=(); 
my %proteomesScoreHash_ALL=(); 
#processProteomes();
#processProteomes_ALL();
...
#sub processProteomes_ALL {
	foreach my $upIdAndTaxId (keys %proteomeUniRefEntryHash_ALL) {
		my $proteome = $proteomesHash_ALL{$upIdAndTaxId};
		print "Proteome: ".$proteome->getUPIdAndTaxId()."\n";
        	print "Mnemonic: ".$proteome->getMnemonic()."\n";
        	print "ScientificName: ".$proteome->getScientificName()."\n";
        	print "Lineage: ".$proteome->getLineage()."\n";
		...
		my %members = ();
		my %uniRef50s;
		foreach my $uniRefAc ( keys %{ $proteomeUniRefEntryHash_ALL{$upIdAndTaxId}}) {
			$uniRef50s{$uniRefAc} = "1";
			my $proteinsRef = $proteomeUniRefEntryHash_ALL{$upIdAndTaxId}{$uniRefAc};	
			my %proteins = %$proteinsRef;
			my $entrySize = keys %proteins;
			foreach my $entryAc (keys %proteins) {
				my $protein = $proteinsHash_ALL{$upIdAndTaxId}{$entryAc};
				if($members{$entryAc} eq "") {
					$members{$entryAc} = $protein;
					$memberCount++;
				}
		...
		$proteome->setMembers(\%members);
		$proteome->setMemberCount($memberCount);
		$score = $upIdAndTaxIdScoresHash_ALL{$upIdAndTaxId};
		$proteome->setEntryTotal($upIdAndTaxIdEntryTotalHash_ALL{$upIdAndTaxId});
		$proteome->setScore($score);
		$scoreKey = sprintf("%d", $score*100000000);
		$scoreKey += 10000000000000;
		$scoreKey .=".".$proteome->getUPIdAndTaxId();
		if($scoreKey) {
			$proteomesScoreHash_ALL{$scoreKey} = $proteome->getUPIdAndTaxId();	
		}
		my $uniRefSize = keys %{ $proteomeUniRefEntryHash_ALL{$upIdAndTaxId}};
		print "Score: ".$proteome->getScore()."\n";
		print "my TotalEntries: ".keys(%members)."\n";
		print "TotalEntries: ".$proteome->getEntryTotal()."\n";
		print "MemberCount: ".$proteome->getMemberCount()."\n";
	...
	print "All Score Hash size: ".(keys %proteomesScoreHash_ALL)."\n";
#}


print "Start reading corr table\n";
...
while($line=<CORRTAB>) {
	...
	chomp($line);
	$corrTableCount++;
	if($corrTableCount % 100000000 eq 0) {
		my $date = `date`;
		chomp($date);
		print $date." ".$corrTableCount." lines have been read\n";
	}
...
open(TAXGROUP, "<", "../data/upIdAndTaxIdToVirusGroupAndHost.txt") or die "Can't open ../data/upIdAndTaxIdToVirusGroupAndHost.txt\n";
while($line=<TAXGROUP>) {
        chomp($line);
        ($upId, $taxId, $taxGroup) = (split(/\t/, $line))[0, 1, 3];
        $upIdAndTaxIdToTaxGroupName{$upId."-".$taxId} = $taxGroup;
}
close(TAXGROUP);
...
#my @loop = (95, 90, 85, 80, 75, 70, 65, 60, 55, 50, 45, 40, 35, 30, 25, 20, 15, 10, 5);
#my @loop = (100);
#my @loop = (99);
#my @loop = (98);
#my @loop = (97);
...
#my @loop = (92);
#my @loop = (91);
#my @loop = (95, 75, 55, 35, 15);
my @loop = (95);

my %inputProteomesScoreHash = ();

foreach(@loop) {
	if($_ == 95) {
                %inputProteomesScoreHash = %sumCorrTaxIdHash95;
                %inputProteomesScoreHash = computeRPGAndPanProteome($_, %inputProteomesScoreHash);
        }
        else {
                %inputProteomesScoreHash = computeRPGAndPanProteome($_, %inputProteomesScoreHash);
        }
...
sub getMeanCorr {
	...
	foreach my $upIdAndTaxId1 (sort keys %upIdAndTaxIdPairsCorrHash95) {
                foreach my $upIdAndTaxId2 (keys %{ $upIdAndTaxIdPairsCorrHash95{$upIdAndTaxId1} }) {
...
sub computeRPGAndPanProteome {
	...
	foreach my $k1 (sort keys(%rpgNameHash)) { 
		...
		my $pproteome1 = $proteomesHash_ALL{$key1};
		...
		my $puniRef50sRef1 = $pproteome1->getUniRef50s();
		my %puniRef50s1 = %$puniRef50sRef1;
		my $pmembersRef1 = $pproteome1->getMembers();
		my %pmembers1 = %$pmembersRef1;
		my %panMembers= ();
		my $upIdAndTaxId1 = $proteome1->getUPIdAndTaxId();
		print "taxMember: ".$upIdAndTaxId1."|||".keys(%members1)."\n";
		foreach my $key1 (keys %pmembers1) {
			$upIdAndTaxEntryHash{$upIdAndTaxId1}{$upIdAndTaxId1}{$key1} = 1;
			my ($upId, $taxonomyId) = (split(/-/, $upIdAndTaxId1))[0, 1];
			$panMembers{$key1} = $upId."\t".$taxonomyId."\t".$entryProteomeHash_ALL{$key1}{$upIdAndTaxId1}; 	
		}
		my %seen = ();
		...
		if($nrpSize > 0) {	
			...
			foreach(@unique) {
				...
				my $pproteome2 = $proteomesHash_ALL{$key2};
				...
				my $puniRef50sRef2 = $pproteome2->getUniRef50s();
				my %uniRef50s2 = %$uniRef50sRef2;
				my %puniRef50s2 = %$puniRef50sRef2;
				my $upIdAndTaxId = $proteome2->getUPIdAndTaxId();
					
				foreach my $uniRefAc (keys %puniRef50s2) {
					if($puniRef50s1{$uniRefAc} eq "") {
						my $proteinsRef = $proteomeUniRefEntryHash_ALL{$upIdAndTaxId}{$uniRefAc};
                        			my %proteins = %$proteinsRef;
			...
			foreach(sort keys %rpgMemberNameHash) {
	 		...
	 		$pproteome1->setPanMembers(\%panMembers);
			$proteomesHash_ALL{$key1} = $pproteome1;
			#print $key1."\t".$pproteome1->getUPId()."\t"."Pan member size before: ". keys($pproteome1->getPanMembers())."\n";
			print $key1."\t".$pproteome1->getUPId()."\t"."Pan member size before: ". keys(%{$pproteome1->getPanMembers()})."\n";
		}
		print RPG "\n";
	}	
	...
	if($Xin == 95) {
		...
		close(RUN95);
		open(RUNNINGRP95, ">", "../data/runningRP95.txt") or die "Can't open ../data/runningRP95.txt\n";	 
		for $key (keys %runningRP) {
			print RUNNINGRP95 $key."\t".$runningRP{$key}."\t".$seedToRP{$key}."\n";
		}	
		close(RUNNINGRP95);
