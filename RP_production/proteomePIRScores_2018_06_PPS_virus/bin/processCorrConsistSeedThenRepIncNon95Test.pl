...
close(TAX);
print "Total ProteomesHash ALL: ".keys(%proteomesHash_ALL)."\n";
print "Getting proteome info ALL ...done\n";
...
print "Getting entry score ALL ...done\n";
...
print "Score Hash size ALL: ".(keys %proteomesScoreHash_ALL)."\n";
...
sub getMeanCorr {
	...
	#}
        ...
        #}
        close(CORRTAB);
	#$upIdAndTaxIdPairsCorrHash

	foreach my $upIdAndTaxId1 (sort keys %upIdAndTaxIdPairsCorrHash) {
    		foreach my $upIdAndTaxId2 (keys %{ $upIdAndTaxIdPairsCorrHash{$upIdAndTaxId1} }) {
	...
	foreach my $k1 (sort keys(%rpgNameHash)) { 
		...
		if($nrpSize > 0) {	
			...
			foreach(@unique) {
				...
				my $pproteome2 = $proteomesHash{$key2};
