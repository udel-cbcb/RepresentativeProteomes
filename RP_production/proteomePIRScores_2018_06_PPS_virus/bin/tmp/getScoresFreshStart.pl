...
if(@ARGV != 0) {
        print "perl getScoresFreshStart.pl\n";
        exit 1;
}
my $firstAC = "";
my $pmidScore = 0;
...
if( ! -d "../data/score_fresh") {
	system("mkdir -p ../data/score_fresh");
}
...
foreach my $upIdAndTaxId (sort keys %upIdAndTaxIdHash) {
	...
	$scoreFile =~ s/\t/-/;
	open(SCORE, ">", "../data/score_fresh/".$scoreFile."_score.txt");
...
open(PSCORE, ">", "../data/score_fresh/proteomeScores.txt");
